...
This data was taken from a nutrition study conducted on three groups of rats. The groups were put on different diets, and each animal's body weight (grams) was recorded repeatedly (approximately) weekly, except in week seven when two recordings were taken) over a 9-week period.
...
Once standardised we can see that the groups are on similar trajecties but maintain significant variance between the different groups.
...
The above plots show the different bprs scores gathered over an 8 week period for the different treatment groups. It's hard to pick up a discernable pattern, however.
...
summary(BPRS_ref)
```
A random intercept is fit so that the lubear regression for each bprs score can differ in intercept for other subjects, thereby accounting for individual differences in the subject profiles.
