...
```{r}
# Load libraries
library(stringr); library(dplyr); library(GGally); library(corrplot); library(ggplot2); library(tidyr); library(MASS)
...
For this analysis the two data sets have been combined by the variable 'country' while variables unnecessary for this analysis have beenr emoved. Additionally, the variable capturing Gross National Income (GNI) has been converted to a numerica variable.
...
ggpairs(human)
gather(human) %>% ggplot(aes(value)) + facet_wrap("key", scales = "free") + geom_density()
...
Based on the plot we can see a relationship between those who buy unpackaged tea in tea shops (and possibly prefer green tea) as well as those who buy tea bags in chain stores. We can also see that both Dim.1 and Dim.2 have something to do with how and where people drink tea, whereas Dim.3 describes the type of tea, how they drink it and amount of sugar.